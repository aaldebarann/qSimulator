...
void Circuit::ccnot(size_t control1, size_t control2, size_t target) {
    // matrices.push_back(CCNOT(control1, control2, target, size));
    h(target);
...
void Circuit::qadd(size_t first, size_t bits, bool approximate) {
    if(2 * bits > size)
        throw std::out_of_range("quantum addition requires 2n bits to add two n-bit numbers");
    ...
    for(int i = 0; i < bits; i++) {
        for(int j = 0; j < bits; j++) {
            ...
            cp(2 * M_PI * tmp, first + j,  first + j + bits - i);
        }
    }
}
void Circuit::iqadd(size_t first, size_t bits, bool approximate) {
    if(2 * bits > size)
        throw std::out_of_range("quantum addition requires 2n bits to add two n-bit numbers");
    int threshold;
    if(approximate) {
        threshold = std::min((int) log2(bits) + 1, (int)bits - 1);
    } else {
        threshold = bits - 1;
    }
...
void Circuit::qaddMod(size_t first, size_t bits, int module, bool approximate) {
